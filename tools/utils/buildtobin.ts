...
function generateSplashHeaderFile(outputPath: string): void {
    //"""Generate header file for splash screen"""
    const headerContent = `#pragma once
...
function generateSplashCppFile(pngData: Uint8Array, outputPath: string, headerName: string, variableName: string): void {
    //"""Generate cpp implementation file for splash screen"""
    const cppArray = bytesToCppArray(pngData, variableName);
...
function main(): void {
    ...
    const splashPngFile = join(projectRoot, 'docs', 'resources', 'splash.png');
    ...
    const splashCppFile = join(appDir, 'splash.cpp');
    const splashHppFile = join(appDir, 'splash.hpp');
    ...
    if (!existsSync(splashPngFile)) {
        console.error(`Error: Input file not found: ${splashPngFile}`);
        console.error("Please ensure the splash screen PNG file exists at docs/resources/splash.png");
        process.exit(1);
    }
    ...
    console.log(`Reading splash screen PNG file: ${splashPngFile}`);
    const splashPngData = readBinaryFile(splashPngFile);

    console.log(`Generating splash screen header file: ${splashHppFile}`);
    generateSplashHeaderFile(splashHppFile);

    console.log(`Generating splash screen implementation file: ${splashCppFile}`);
    generateSplashCppFile(splashPngData, splashCppFile, 'splash.hpp', 'splash_png_data');
    ...
    console.log(`  - ${splashHppFile} (${statSync(splashHppFile).size} bytes)`);
    console.log(`  - ${splashCppFile} (${statSync(splashCppFile).size} bytes)`);
    ...
    console.log(`Splash Screen PNG data size: ${splashPngData.length} bytes`);
    ...
    console.log('\nFor splash screen:');
    console.log('  #include "splash.hpp"');
    console.log('  const unsigned char* data = GetSplashScreenData();');
    console.log('  unsigned int size = GetSplashScreenSize();');
}
