...
DX11Renderer::DX11Renderer()
    ...
    , vsyncEnabled_(true)
    , multiSampleCount_(1)
    , textureFormat_(DXGI_FORMAT_B8G8R8A8_UNORM)
    , frameTime_(0.0)
    , frameCount_(0)
...
bool DX11Renderer::Initialize(HWND hwnd, int width, int height) {
    ...
    Logger::LogMessage("DX11Renderer: Initializing " + std::to_string(width) + "x" + std::to_string(height));
...
bool DX11Renderer::CreateDevice() {
    ...
    Logger::LogMessage("DX11Renderer: Created device with feature level " + std::to_string(featureLevel >> 12) + "." + std::to_string((featureLevel >> 8) & 0xF));
...
bool DX11Renderer::CreateRenderTargets() {
    ...
    hr = device_->CreateRenderTargetView(backBuffer_.Get(), nullptr, &renderTargetView_);
    if (FAILED(hr)) {
...
bool DX11Renderer::CreateShaders() {
    ...
    if (!CompileShaderFromString(DX11Shaders::VERTEX_SHADER_SOURCE, "main", "vs_5_0", &vsBlob)) {
    ...
    if (!CompileShaderFromString(DX11Shaders::PIXEL_SHADER_SOURCE, "main", "ps_5_0", &psBlob)) {
...
bool DX11Renderer::CreateGeometry() {
    ...
    HRESULT hr = device_->CreateBuffer(&vertexBufferDesc, &vertexData, &vertexBuffer_);
    if (FAILED(hr)) {
...
bool DX11Renderer::CreateStates() {
    ...
    depthStencilDesc.DepthEnable = FALSE;  // Disable depth testing for 2D rendering
...
bool DX11Renderer::CreateCEFTexture(int width, int height) {
    ...
    hr = device_->CreateShaderResourceView(cefTexture_.Get(), &srvDesc, &cefTextureView_);
    if (FAILED(hr)) {
        LogError("Failed to create CEF texture view", hr);
        return false;
    }
    
    Logger::LogMessage("DX11Renderer: Created CEF texture " + std::to_string(width) + "x" + std::to_string(height));
    return true;
}

bool DX11Renderer::CreateTextureFromBuffer(const void* buffer, int width, int height) {
...
bool DX11Renderer::EndFrame() {
    if (!initialized_ || !cefTextureView_) return false;
...
bool DX11Renderer::ResizeTextures(int width, int height) {
    ...
    Logger::LogMessage("DX11Renderer: Resizing to " + std::to_string(width) + "x" + std::to_string(height));
...
void DX11Renderer::EnableVSync(bool enable) {
    ...
    Logger::LogMessage("DX11Renderer: VSync " + std::string(enable ? "enabled" : "disabled"));
}

void DX11Renderer::SetMultiSampleCount(int samples) {
    if (samples != multiSampleCount_) {
        multiSampleCount_ = samples;
        Logger::LogMessage("DX11Renderer: Multisampling set to " + std::to_string(samples) + "x");
...
bool DX11Renderer::CheckFeatureSupport() {
    ...
    device_->CheckFeatureSupport(D3D11_FEATURE_THREADING, &threadingSupport, sizeof(threadingSupport));
    
    Logger::LogMessage("DX11Renderer: Driver concurrent creates: " + std::string(threadingSupport.DriverConcurrentCreates ? "Yes" : "No"));
    Logger::LogMessage("DX11Renderer: Driver command lists: " + std::string(threadingSupport.DriverCommandLists ? "Yes" : "No"));
    
    return true;
}

std::string DX11Renderer::GetAdapterInfo() {
    if (!factory_) return "No adapter info available";
    
    ComPtr<IDXGIAdapter1> adapter;
    HRESULT hr = factory_->EnumAdapters1(0, &adapter);
    if (FAILED(hr)) return "Failed to enumerate adapter";
    
    DXGI_ADAPTER_DESC1 desc;
    hr = adapter->GetDesc1(&desc);
    if (FAILED(hr)) return "Failed to get adapter description";
...
void DX11Renderer::LogPerformanceStats() {
    if (frameCount_ > 0) {
        Logger::LogMessage("DX11Renderer: Frame time: " + std::to_string(frameTime_) + "ms, FPS: " + std::to_string(1000.0 / frameTime_));
    }
}
...
bool DX11Renderer::CompileShaderFromString(const std::string& shaderSource, const std::string& entryPoint, 
    ...
    if (FAILED(hr)) {
        if (errorBlob) {
            std::string error(static_cast<const char*>(errorBlob->GetBufferPointer()), errorBlob->GetBufferSize());
...
void DX11Renderer::EnablePartialUpdates(bool enable) {
    ...
    Logger::LogMessage("DX11Renderer: Partial updates " + std::string(enable ? "enabled" : "disabled"));
}
