...
public:
    ...
    // Sandbox management
    std::unique_ptr<SandboxProcess> CreateSandboxProcess(
    ...
    // Project management
    bool CreateProject(const std::string& name, const std::string& toolchainId, const std::string& path);
    bool OpenProject(const std::string& path);
    bool CloseProject();
    std::string GetCurrentProjectPath() const { return m_currentProjectPath; }

    // Build and execution
    std::unique_ptr<SandboxProcess> BuildProject(const SandboxConfig& config = {});
    std::unique_ptr<SandboxProcess> RunProject(const SandboxConfig& config = {});
    std::unique_ptr<SandboxProcess> ExecuteCommand(
    ...
    // Event callbacks
    using ToolchainEventCallback = std::function<void(const std::string&, const std::string&)>;
    void SetOnToolchainRegistered(ToolchainEventCallback callback) { m_onToolchainRegistered = callback; }
    void SetOnToolchainUnregistered(ToolchainEventCallback callback) { m_onToolchainUnregistered = callback; }
    void SetOnProjectOpened(ToolchainEventCallback callback) { m_onProjectOpened = callback; }
    void SetOnProjectClosed(ToolchainEventCallback callback) { m_onProjectClosed = callback; }
