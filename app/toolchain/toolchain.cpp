...
public:
  WindowsSandboxProcess(const std::string &command,
                        const std::vector<std::string> &args,
                        const SandboxConfig &config)
      : m_processHandle(nullptr), m_threadHandle(nullptr), m_processId(0),
        m_config(config), m_startTime(0) {
  ...
  bool IsRunning() const override {
    if (!m_processHandle)
      return false;
    DWORD exitCode;
    return GetExitCodeProcess(m_processHandle, &exitCode) &&
  ...
  ExecutionResult GetResult() override {
    ...
    if (GetProcessMemoryInfo(m_processHandle, &pmc, sizeof(pmc))) {
      ...
      if (m_config.memoryLimit > 0 &&
          result.memoryUsed > m_config.memoryLimit) {
    ...
    if (m_config.timeLimit > 0 &&
        result.executionTime > m_config.timeLimit * 1000) {
  ...
  void SendInput(const std::string &input) override {
    if (m_stdinWrite) {
      DWORD written;
      WriteFile(m_stdinWrite, input.c_str(), static_cast<DWORD>(input.length()),
                &written, nullptr);
    }
  }

  std::string ReadOutput() override { return ReadFromPipe(m_stdoutRead); }

  std::string ReadError() override { return ReadFromPipe(m_stderrRead); }
...
ToolchainManager::~ToolchainManager() { Shutdown(); }
...
void ToolchainManager::SetUI(WindowedUI *ui) { m_ui = ui; }
...
bool ToolchainManager::RegisterToolchain(const ToolchainInfo &toolchain) {
  ...
  std::cout << "Registered toolchain: " << toolchain.name << " ("
            << toolchain.id << ")" << std::endl;
  return true;
}
...
bool ToolchainManager::UnregisterToolchain(const std::string &toolchainId) {
  ...
  std::cout << "Unregistered toolchain: " << name << " (" << toolchainId << ")"
            << std::endl;
  return true;
}
...
std::unique_ptr<SandboxProcess> ToolchainManager::CreateSandboxProcess(
    const std::string &toolchainId, const std::vector<std::string> &arguments,
    const SandboxConfig &config) {
  ...
  auto process = std::make_unique<WindowsSandboxProcess>(
      toolchain->executablePath, arguments, config);
  return process;
}

bool ToolchainManager::CreateProject(const std::string &name,
...
ToolchainManager::BuildProject(const SandboxConfig &config) {
  if (m_currentProjectPath.empty()) {
    return nullptr;
  }
...
std::unique_ptr<SandboxProcess>
ToolchainManager::RunProject(const SandboxConfig &config) {
  if (m_currentProjectPath.empty()) {
    return nullptr;
  }
...
std::unique_ptr<SandboxProcess>
ToolchainManager::ExecuteCommand(const std::string &command,
  ...
  auto process =
      std::make_unique<WindowsSandboxProcess>(command, arguments, config);
  return process;
}
...
void ToolchainManager::DetectSystemToolchains() {
  ...
  std::vector<std::pair<std::string, std::string>> commonTools = {
      {"gcc", "gcc.exe"},       {"clang", "clang.exe"}, {"msvc", "cl.exe"},
      {"python", "python.exe"}, {"node", "node.exe"},   {"java", "java.exe"},
      {"go", "go.exe"},         {"rust", "rustc.exe"}};
