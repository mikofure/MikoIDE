...
void UILabel::Render(ID2D1RenderTarget *renderTarget,
                     IDWriteTextFormat *textFormat,
                     IDWriteTextFormat *boldTextFormat,
                     IDWriteTextFormat *regularTextFormat,
                     IDWriteTextFormat *semiboldCenteredTextFormat) {
  if (!IsVisible() || !renderTarget || !textFormat)
    return;

  ID2D1SolidColorBrush *brush = nullptr;
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.9f, 0.9f, 0.9f, 1.0f),
                                      &brush);

  if (brush) {
    D2D1_RECT_F rect =
        D2D1::RectF(m_bounds.x, m_bounds.y, m_bounds.x + m_bounds.width,
...
void UIList::Render(ID2D1RenderTarget *renderTarget,
                    IDWriteTextFormat *textFormat,
                    IDWriteTextFormat *boldTextFormat,
                    IDWriteTextFormat *regularTextFormat,
                    IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.15f, 0.15f, 0.15f, 1.0f),
                                      &brush);
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.3f, 0.7f, 1.0f, 0.3f),
                                      &selectionBrush);
  ...
  if (brush && selectionBrush) {
    ...
    for (size_t i = m_scrollOffset;
         i < m_items.size() && currentY < m_bounds.y + m_bounds.height; ++i) {
  ...
  if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
    if (event.button.button == SDL_BUTTON_LEFT) {
      ...
      if (m_bounds.Contains(mouseX, mouseY)) {
        float itemHeight = 25.0f;
        int clickedIndex =
            static_cast<int>((mouseY - m_bounds.y - 5.0f) / itemHeight) +
            m_scrollOffset;

        if (clickedIndex >= 0 &&
            clickedIndex < static_cast<int>(m_items.size())) {
...
void UIList::AddItem(const std::string &item) { m_items.push_back(item); }
...
void UITextInput::Render(ID2D1RenderTarget *renderTarget,
                         IDWriteTextFormat *textFormat,
                         IDWriteTextFormat *boldTextFormat,
                         IDWriteTextFormat *regularTextFormat,
                         IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.2f, 0.2f, 0.2f, 1.0f),
                                      &brush);
  renderTarget->CreateSolidColorBrush(
      m_focused ? D2D1::ColorF(0.2f, 0.6f, 1.0f, 1.0f)
...
void UIPanel::Render(ID2D1RenderTarget *renderTarget,
                     IDWriteTextFormat *textFormat,
                     IDWriteTextFormat *boldTextFormat,
                     IDWriteTextFormat *regularTextFormat,
                     IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  for (auto &widget : m_widgets) {
    if (widget && widget->IsVisible()) {
      widget->Render(renderTarget, textFormat, boldTextFormat,
                     regularTextFormat, semiboldCenteredTextFormat);
    }
  }
}
...
void UIPanel::Clear() { m_widgets.clear(); }

// WindowedUI Implementation
WindowedUI::WindowedUI()
    : m_window(nullptr), m_renderer(nullptr), m_hwnd(nullptr),
      m_menuBar(nullptr), m_originalWndProc(nullptr), m_d2dFactory(nullptr),
      m_renderTarget(nullptr), m_brush(nullptr), m_writeFactory(nullptr),
      m_textFormat(nullptr), m_focusedWidget(nullptr), m_width(1200),
      m_height(800), m_initialized(false), m_minimized(false),
      m_header(nullptr), m_sidebar(nullptr), m_content(nullptr) {}

WindowedUI::~WindowedUI() { Shutdown(); }
...
bool WindowedUI::InitializeSDL(const std::string &title, int width,
                               int height) {
...
bool WindowedUI::InitializeDirect2D() {
  ...
  hr = m_renderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White),
                                             &m_brush);
...
bool WindowedUI::InitializeDirectWrite() {
  ...
  hr = m_writeFactory->CreateTextFormat(
      ...
      DWRITE_FONT_STRETCH_NORMAL, 14.0f, L"", &m_textFormat);
  ...
  hr = m_writeFactory->CreateTextFormat(
      L"Segoe UI", nullptr, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
      DWRITE_FONT_STRETCH_NORMAL, 14.0f, L"", &m_centeredTextFormat);
  ...
  m_centeredTextFormat->SetParagraphAlignment(
  ...
  m_semiboldCenteredTextFormat->SetParagraphAlignment(
      DWRITE_PARAGRAPH_ALIGNMENT_CENTER);

  // Create regular text format for path text
  hr = m_writeFactory->CreateTextFormat(
      L"Segoe UI", nullptr, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
      DWRITE_FONT_STRETCH_NORMAL, 14.0f, L"", &m_regularTextFormat);
...
void WindowedUI::CreateUIElements() {
  ...
  auto gccCard = std::make_shared<UIToolchainCard>(
      ...
      "Objective-C, Fortran, Ada, Go, and D",
      "Compiler");
...
void WindowedUI::SetupMainLayout() { UpdateLayout(); }

void WindowedUI::UpdateLayout() {
  if (!m_header || !m_sidebar || !m_content)
    return;
  ...
  int headerHeight = 80;
  m_header->SetBounds({0.0f, 0.0f, static_cast<float>(width),
  ...
  m_sidebar->SetBounds({0.0f, static_cast<float>(bodyY),
                        static_cast<float>(sidebarWidth),
                        static_cast<float>(bodyHeight)});

  // Content layout (3/4 width on right side of body)
  int contentX = sidebarWidth;
  int contentWidth = width - sidebarWidth;
  m_content->SetBounds({static_cast<float>(contentX), static_cast<float>(bodyY),
...
void WindowedUI::BeginFrame() {
  ...
  if (m_renderTarget) {
    ...
    m_renderTarget->Clear(
        ColorFromTheme(m_theme.background.r, m_theme.background.g,
...
void WindowedUI::RenderWidgets() {
  for (auto &widget : m_widgets) {
    if (widget && widget->IsVisible()) {
      ...
      if (widget == m_header && m_boldTextFormat && m_regularTextFormat &&
          m_semiboldCenteredTextFormat) {
        // Cast to UIHeader to call the extended Render method
        auto header = std::dynamic_pointer_cast<UIHeader>(widget);
        if (header) {
          header->Render(m_renderTarget, m_textFormat, m_boldTextFormat,
                         m_regularTextFormat, m_semiboldCenteredTextFormat);
        } else {
          widget->Render(m_renderTarget, m_textFormat, m_boldTextFormat,
                         m_regularTextFormat, m_semiboldCenteredTextFormat);
        }
      } else {
...
D2D1_COLOR_F WindowedUI::ColorFromTheme(float r, float g, float b,
                                        float a) const {
...
void WindowedUI::ClearWidgets() { m_widgets.clear(); }
...
bool WindowedUI::IsMinimized() const { return m_minimized; }
...
bool WindowedUI::CreateMenuBar() {
  ...
  AppendMenuW(fileMenu, MF_STRING, ID_FILE_NEW_PROJECT,
              L"&New Project\tCtrl+N");
  AppendMenuW(fileMenu, MF_STRING, ID_FILE_OPEN_PROJECT,
              L"&Open Project\tCtrl+O");
  AppendMenuW(fileMenu, MF_SEPARATOR, 0, nullptr);
  AppendMenuW(fileMenu, MF_STRING, ID_FILE_SAVE_PROJECT,
  ...
  AppendMenuW(buildMenu, MF_STRING, ID_BUILD_REBUILD,
              L"&Rebuild All\tCtrl+Alt+F7");
  ...
  AppendMenuW(buildMenu, MF_SEPARATOR, 0, nullptr);
  AppendMenuW(buildMenu, MF_STRING, ID_BUILD_CONFIGURATION,
              L"&Configuration Manager");
  AppendMenuW(m_menuBar, MF_POPUP, (UINT_PTR)buildMenu, L"&Build");

  // Create Tools menu
  HMENU toolsMenu = CreatePopupMenu();
  AppendMenuW(toolsMenu, MF_STRING, ID_TOOLS_TOOLCHAIN_MANAGER,
              L"&Toolchain Manager");
  AppendMenuW(toolsMenu, MF_STRING, ID_TOOLS_PACKAGE_MANAGER,
              L"&Package Manager");
  ...
  AppendMenuW(helpMenu, MF_STRING, ID_HELP_KEYBOARD_SHORTCUTS,
  ...
  m_originalWndProc =
      (WNDPROC)SetWindowLongPtr(m_hwnd, GWLP_WNDPROC, (LONG_PTR)WindowProc);
...
void WindowedUI::HandleMenuCommand(WPARAM wParam) {
  ...
  case ID_HELP_ABOUT:
    MessageBoxW(m_hwnd,
                L"Hyperion Toolchain Manager\nVersion 1.0\n\nBuilt with SDL3, "
                L"Direct2D, and DirectWrite",
                L"About Hyperion Toolchain", MB_OK | MB_ICONINFORMATION);
    break;
  }
}

LRESULT CALLBACK WindowedUI::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                        LPARAM lParam) {
...
void UIButton::Render(ID2D1RenderTarget *renderTarget,
  ...
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(bgR, bgG, bgB, bgA),
                                      &bgBrush);
  renderTarget->CreateSolidColorBrush(
      D2D1::ColorF(m_textColor.r, m_textColor.g, m_textColor.b, m_textColor.a),
      &textBrush);
  ...
  if (bgBrush && textBrush) {
    ...
    renderTarget->DrawText(
        wtext.c_str(), static_cast<UINT32>(wtext.length()), textFormat, bgRect,
        textBrush, D2D1_DRAW_TEXT_OPTIONS_NONE, DWRITE_MEASURING_MODE_NATURAL);
...
void UIButton::SetStyle(const std::string &bgColor,
...
void UIHeader::Render(ID2D1RenderTarget *renderTarget,
                      IDWriteTextFormat *textFormat,
                      IDWriteTextFormat *boldTextFormat,
                      IDWriteTextFormat *regularTextFormat,
                      IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.9f, 0.9f, 0.9f, 1.0f),
                                      &textBrush);
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.1f, 0.1f, 0.1f, 1.0f),
                                      &bgBrush);
  ...
  if (textBrush && bgBrush) {
    ...
    D2D1_RECT_F pathRect =
        D2D1::RectF(m_bounds.x + 20, m_bounds.y + 35,
                    m_bounds.x + m_bounds.width - 200, m_bounds.y + 55);
    std::wstring pathText(m_path.begin(), m_path.end());
    renderTarget->DrawText(pathText.c_str(),
                           static_cast<UINT32>(pathText.length()),
                           regularTextFormat ? regularTextFormat : textFormat,
                           pathRect, textBrush);

    // Position and render the change directory button
    float buttonWidth = 150;
    float buttonHeight = 35;
    m_changeDirectoryButton->SetBounds(
        Rect(m_bounds.x + m_bounds.width - buttonWidth - 20, m_bounds.y + 15,
             buttonWidth, buttonHeight));
    m_changeDirectoryButton->Render(
        renderTarget,
        semiboldCenteredTextFormat ? semiboldCenteredTextFormat : textFormat,
        boldTextFormat, regularTextFormat, semiboldCenteredTextFormat);
...
void UICategoryItem::Render(ID2D1RenderTarget *renderTarget,
                            IDWriteTextFormat *textFormat,
                            IDWriteTextFormat *boldTextFormat,
                            IDWriteTextFormat *regularTextFormat,
                            IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.9f, 0.9f, 0.9f, 1.0f),
                                      &textBrush);
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.6f, 0.6f, 0.6f, 1.0f),
                                      &badgeBrush);

  // Background color based on state
  if (m_selected) {
    renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.2f, 0.6f, 1.0f, 0.3f),
                                        &bgBrush);
  } else if (m_hovered) {
    renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.2f, 0.2f, 0.2f, 1.0f),
                                        &bgBrush);
  ...
  if (textBrush && bgBrush && badgeBrush) {
    ...
    D2D1_RECT_F nameRect = D2D1::RectF(m_bounds.x + 15, m_bounds.y + 8,
                                       m_bounds.x + m_bounds.width - 50,
                                       m_bounds.y + m_bounds.height - 8);
    std::wstring nameText(m_name.begin(), m_name.end());
    renderTarget->DrawText(nameText.c_str(),
                           static_cast<UINT32>(nameText.length()), textFormat,
                           nameRect, textBrush);

    // Draw count badge
    D2D1_RECT_F badgeRect = D2D1::RectF(
        m_bounds.x + m_bounds.width - 40, m_bounds.y + 8,
        m_bounds.x + m_bounds.width - 10, m_bounds.y + m_bounds.height - 8);
    std::wstring countText = std::to_wstring(m_count);
    renderTarget->DrawText(countText.c_str(),
                           static_cast<UINT32>(countText.length()), textFormat,
                           badgeRect, badgeBrush);
...
void UISidebar::Render(ID2D1RenderTarget *renderTarget,
                       IDWriteTextFormat *textFormat,
                       IDWriteTextFormat *boldTextFormat,
                       IDWriteTextFormat *regularTextFormat,
                       IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  ID2D1SolidColorBrush *bgBrush = nullptr;
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.12f, 0.12f, 0.12f, 1.0f),
                                      &bgBrush);
  ...
  if (bgBrush) {
    ...
    for (auto &category : m_categories) {
      category->SetBounds(
          Rect(m_bounds.x, currentY, m_bounds.width, itemHeight));
      category->Render(renderTarget, textFormat, boldTextFormat,
                       regularTextFormat, semiboldCenteredTextFormat);
...
UIToolchainCard::UIToolchainCard(const std::string &name,
...
void UIToolchainCard::Render(ID2D1RenderTarget *renderTarget,
                             IDWriteTextFormat *textFormat,
                             IDWriteTextFormat *boldTextFormat,
                             IDWriteTextFormat *regularTextFormat,
                             IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.9f, 0.9f, 0.9f, 1.0f),
                                      &textBrush);
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.18f, 0.18f, 0.18f, 1.0f),
                                      &bgBrush);
  renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.2f, 0.6f, 1.0f, 1.0f),
                                      &badgeBrush);
  ...
  if (textBrush && bgBrush && badgeBrush) {
    ...
    renderTarget->CreateSolidColorBrush(D2D1::ColorF(0.4f, 0.4f, 0.4f, 1.0f),
                                        &iconBrush);
    if (iconBrush) {
      D2D1_ELLIPSE iconEllipse = D2D1::Ellipse(
          D2D1::Point2F(m_bounds.x + 30, m_bounds.y + 30), 20, 20);
    ...
    D2D1_RECT_F nameRect =
        D2D1::RectF(m_bounds.x + 70, m_bounds.y + 15,
    ...
    renderTarget->DrawText(badgeText.c_str(),
                           static_cast<UINT32>(badgeText.length()), textFormat,
                           badgeRect, textBrush);

    // Position and render install button
    float buttonWidth = 70;
    float buttonHeight = 30;
    m_installButton->SetBounds(
        Rect(m_bounds.x + m_bounds.width - buttonWidth - 10,
             m_bounds.y + m_bounds.height - buttonHeight - 10, buttonWidth,
             buttonHeight));
    m_installButton->Render(renderTarget, textFormat, boldTextFormat,
                            regularTextFormat, semiboldCenteredTextFormat);
...
void UIContent::Render(ID2D1RenderTarget *renderTarget,
                       IDWriteTextFormat *textFormat,
                       IDWriteTextFormat *boldTextFormat,
                       IDWriteTextFormat *regularTextFormat,
                       IDWriteTextFormat *semiboldCenteredTextFormat) {
  ...
  if (bgBrush) {
    ...
    float currentY =
        m_bounds.y + 20 - (m_scrollOffset * (cardHeight + cardSpacing));

    for (auto &card : m_cards) {
      if (currentY + cardHeight > m_bounds.y &&
          currentY < m_bounds.y + m_bounds.height) {
        card->SetBounds(
            Rect(m_bounds.x + 20, currentY, m_bounds.width - 40, cardHeight));
        card->Render(renderTarget, textFormat, boldTextFormat,
                     regularTextFormat, semiboldCenteredTextFormat);
      }
...
void UIContent::ClearCards() { m_cards.clear(); }
