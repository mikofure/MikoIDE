        ...
        switch (command) {
            ...
            case 'f': {
                ...
                if (std::getline(iss, token, ';')) {
                    if (!token.empty()) row = std::stoi(token);
                }
                if (std::getline(iss, token, ';')) {
                    if (!token.empty()) col = std::stoi(token);
                }
        ...
        switch (code) {
            ...
            case 38: {
                ...
                if (std::getline(iss, mode, ';') && mode == "2" &&
                    ...
                    std::getline(iss, g, ';') &&
                    std::getline(iss, b, ';')) {
                    m_currentForeground = RGBColor(std::stoi(r), std::stoi(g), std::stoi(b));
                }
                break;
            }
            ...
            case 48: {
                ...
                if (std::getline(iss, mode, ';') && mode == "2" &&
                    ...
                    std::getline(iss, g, ';') &&
                    std::getline(iss, b, ';')) {
                    m_currentBackground = RGBColor(std::stoi(r), std::stoi(g), std::stoi(b));
                }
                break;
            }
        }
...
void TerminalBuffer::Backspace() {
    ...
    if (m_promptEndX >= 0 && m_promptEndY >= 0) {
        // If we're on the prompt line and at or before the prompt end position, ignore backspace
