...
static const std::map<std::string, int> resourceMap = {
    ...
    {"/monaco/index.html",
     IDR_HTML_EDITOR}, // Add monaco subdomain mapping to editor resource
    {"/menuoverlay/index.html",
     IDR_HTML_MENUOVERLAY}, // Add menuoverlay subdirectory mapping
...
CreateResourceReader(const std::vector<uint8_t> &data) {
  if (data.empty()) {
    return nullptr;
  }
