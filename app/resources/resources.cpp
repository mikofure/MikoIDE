...
CefRefPtr<CefResourceHandler> BinaryResourceProvider::Create(
  ...
  if (url.find("miko://") != 0) {
    Logger::LogMessage(
        "BinaryResourceProvider: URL does not start with miko://");
    return nullptr;
  }
  ...
  if (path.find("app") == 0) {
    ...
    if (path.find("menuoverlay/") == 0) {
      path =
          "/" +
          path; // Convert "menuoverlay/index.html" to "/menuoverlay/index.html"
    } else if (path == "menuoverlay") {
      path = "/menuoverlay/index.html"; // Default to index.html for bare
                                        // menuoverlay
    }
  } else {
  ...
  Logger::LogMessage("BinaryResourceProvider: Resource ID for path '" + path +
                     "': " + std::to_string(resource_id));
  if (resource_id == -1) {
    Logger::LogMessage(
        "BinaryResourceProvider: ERROR - Resource not found for path: " + path);
    Logger::LogMessage(
        "BinaryResourceProvider: Available paths: /index.html, /editor.html, "
        "/menuoverlay/index.html, /main.css, /main.js");
    return nullptr; // Resource not found
  }

  // Load resource data
  std::vector<uint8_t> resource_data =
      ResourceUtil::LoadBinaryResource(resource_id);
  ...
  CefRefPtr<CefStreamReader> stream =
      ResourceUtil::CreateResourceReader(resource_data);
  if (!stream) {
    return nullptr;
  }
